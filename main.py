...
if not tem_aberto:
   ...
   df = pd.DataFrame(columns=[
        ...
        "identificador", "unidade", "modelo", "patrimonio"
    ])
...
# Agora suas contagens e percentuais devem usar o df_filtrado 👇
    ...
    # Gráfico 4 - Seriais por Torre (Todos os Status)
    df4 = df_filtrado.groupby(["group_user", "username"]).size().reset_index(name="quantidade")
    fig4 = px.bar(df4, x="group_user", y="quantidade", color="username", barmode="group",text='quantidade',
                title="Total de Seriais por PA e Torre (Todos os Status)", color_discrete_map=mapa_cores_username,labels={"group_user": "PA", "total_seriais": "Total de Seriais", "status_lote_label": "Status","username":"Torre"})
    fig4.update_traces(textposition='inside', textfont_color='white',textfont_size=50)
    fig4.update_layout(
        plot_bgcolor='rgba(0,0,0,0)',        # fundo do gráfico
        paper_bgcolor='rgba(0,0,0,0)',       # fundo da área externa
        font=dict(color=cor_texto_menu),    # cor dos textos do gráfico
